...
import ComboRationaleCard from '../components/ayesha/ComboRationaleCard';
import DDRSubVectorCard from '../components/ayesha/DDRSubVectorCard';
import HRDUnlockPanel from '../components/ayesha/HRDUnlockPanel';
...
export default function AyeshaCompleteCare() {
  ...
  // S36: No static SL fallback — only use real API data
  // When backend does not return synthetic_lethality, show honest empty state
  const effectiveSL = result?.synthetic_lethality || null;
    ...
    <Box sx={{ p: 4, maxWidth: 1400, mx: 'auto' }}>
      ...
      {result && (
        <Box>
          ...
          {/* HRD Unlock Panel — from completeness data */}
          {result.levels?.L1?.completeness && (
            <Box sx={{ mb: 3 }}>
              <HRDUnlockPanel
                completeness={result.levels?.L1?.completeness}
                testsNeeded={result.tests_needed}
              />
            </Box>
          )}
