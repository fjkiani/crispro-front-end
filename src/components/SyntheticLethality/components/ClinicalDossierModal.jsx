...
### Broken Pathways
${pathways.broken_pathways?.length > 0 ? pathways.broken_pathways.map(p => `- ❌ ${p}`).join('\n') : '- None identified'}

### Essential Backup Pathways
${pathways.essential_pathways?.length > 0 ? pathways.essential_pathways.map(p => `- ⚠️ ${p}`).join('\n') : '- None identified'}
...
function generateInterpretation(results, mutations) {
  ...
  if (highEss.length > 0) {
    parts.push(`**High Essentiality Detected:** ${highEss.map(e => e.gene).join(', ')} show high essentiality scores (≥70%), indicating complete or near-complete loss of function.`);
  }
