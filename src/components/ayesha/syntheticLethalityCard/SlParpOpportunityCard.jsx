...
export function SlParpOpportunityCard({
  ...
  // flag is true OR there are drugs with `matrix_axis === 'parp_inhibitors'`
  // in recommended_drugs. AK ships matrix_axis values without populating
  // essential_pathways.
  if (!hasParpAxis && (!parpDrugs || parpDrugs.length === 0)) {
    return (
      <Box sx={{ p: 1.5, borderRadius: 2, border: '1px solid #e2e8f0', bgcolor: '#ffffff' }}>
        <Typography sx={{ fontWeight: 900 }}>PARP axis</Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
          Not present in this SL payload (no HR/PARP in essential_pathways and no
