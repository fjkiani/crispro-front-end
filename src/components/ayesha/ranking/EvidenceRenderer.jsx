                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {rationale.map((item, rIdx) => {
                        ...
                        if (item.type === 'sequence') {
                            // Backend may provide both a tiny raw `value` (e.g., min_delta proxy) and a normalized `percentile`.
                            // Patient-facing display should prefer `percentile` when present to avoid misleading 0% rounding.
                            const pct = typeof item.percentile === 'number' ? item.percentile : item.value;
                            return (
                                <Typography key={rIdx} variant="body2">
                                    🔬 <strong>Genetic Alignment:</strong> {Math.round((pct || 0) * 100)}%
                                </Typography>
                            );
                        }
