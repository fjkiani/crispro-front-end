...
def test_cytidine_in_vitro_has_pmid():
    receipts = get_literature_receipts("MBD4", CandidateAxis.CYTIDINE_ANALOGS)
    assert "36323843" in receipts["in_vitro"].pmids
...
# ─────────────────────────────────────────────────────────────────────────────
# 6. Zo-validated correction tests (manager-required)
# ─────────────────────────────────────────────────────────────────────────────

def test_parp_gdsc_is_negative():
    """
    Issue A: PARP GDSC2 must be NEGATIVE, not MISSING.
    Our GDSC2 analysis proved: MBD4 LOF (n=8) vs WT (n=478) → ΔZ = −0.024,
    95% CI [−0.84, +0.62]. Spans zero — interrogated and negative.
    """
    receipts = get_literature_receipts("MBD4", CandidateAxis.PARP_INHIBITORS)
    assert "gdsc" in receipts, "GDSC receipt missing for PARP — must be present as NEGATIVE"
    assert receipts["gdsc"].status == ModalityStatus.NEGATIVE, (
        f"GDSC PARP status was {receipts['gdsc'].status}, expected NEGATIVE"
    )
    assert receipts["gdsc"].n_mut == 8
    assert receipts["gdsc"].n_wt == 478


def test_io_clinical_is_positive():
    """
    Issue B: ICB clinical for MBD4 must be POSITIVE, not MIXED.
    Saint-Ghislain 2022 (PMID 35863105): MBD4 deficiency is predictive of
    ICB response in metastatic uveal melanoma.
    """
    receipts = get_literature_receipts("MBD4", CandidateAxis.IMMUNOTHERAPY)
    assert "clinical" in receipts
    assert receipts["clinical"].status == ModalityStatus.POSITIVE, (
        f"IO clinical status was {receipts['clinical'].status}, expected POSITIVE"
    )
    assert "35863105" in receipts["clinical"].pmids, (
        f"Expected PMID 35863105 in IO clinical receipts, got: {receipts['clinical'].pmids}"
    )


def test_cytidine_pmid_correct():
    """
    Issue C: Cytidine analog PMID must be 36323843 (Chabot 2022, npj),
    NOT 35428381. Verify across all cytidine receipt modalities.
    """
    receipts = get_literature_receipts("MBD4", CandidateAxis.CYTIDINE_ANALOGS)
    for key in ("in_vitro", "in_vivo", "clinical", "expression"):
        assert key in receipts, f"Missing {key} receipt for cytidine"
        if receipts[key].pmids:
            assert "36323843" in receipts[key].pmids, (
                f"Cytidine {key} has wrong PMID: {receipts[key].pmids}"
            )
            assert "35428381" not in receipts[key].pmids, (
                f"Cytidine {key} still has old PMID 35428381"
            )


def test_parp_crispr_is_negative():
    """
    Issue D: Frozen CRISPR receipt for PARP1 must be NEGATIVE.
    DepMap 24Q4: Δdep=+0.03, p=0.67, FDR=0.95, d=0.20.
    """
    receipts = get_literature_receipts("MBD4", CandidateAxis.PARP_INHIBITORS)
    assert "crispr" in receipts, "Frozen CRISPR receipt missing for PARP"
    assert receipts["crispr"].status == ModalityStatus.NEGATIVE
    assert receipts["crispr"].delta_dep == 0.03


def test_wrn_crispr_is_negative_and_confound_flagged():
    """
    Issue D: Frozen CRISPR receipt for WRN must be NEGATIVE + confound-flagged.
    DepMap 24Q4 after MSI stratification: p=0.70, d=0.05.
    """
    receipts = get_literature_receipts("MBD4", CandidateAxis.WRN)
    assert "crispr" in receipts, "Frozen CRISPR receipt missing for WRN"
    assert receipts["crispr"].status == ModalityStatus.NEGATIVE
    assert receipts["crispr"].is_confound_flagged is True
